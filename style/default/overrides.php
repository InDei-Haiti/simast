	...
	// tabbed / flat view options
		...
		// tabbed view
			...
			foreach( $this->tabs as $k => $v ) {
				...
				if($k == $this->active){
					$sel = 'Selected';
                    $style = 'style="background: #fff;color:#000;width:auto;height:15px;"';
                    $border_bottom = 'border-bottom: 4px solid #000';
				}else{
					$sel = '';

                    $style = 'style="background: #fff;color:#000;width:auto;height:25px;"';
				}

				$s .= '<!--<td height="28" valign="middle" width="3"><img src="./style/' . $uistyle . '/images/tab'.$sel.'Left.png" width="3" height="28" border="0" alt="" /></td>-->';
				$s .= '<td id="toptab_'.$k.'" valign="middle" nowrap="nowrap" '.$style;
				if ($js_tabs)
					$s .= " class=\"$class\"";
				else $s .='';
				...
				$s .='" style="text-decoration: none;"><span style="margin-left:0px;margin-right:10px;margin-bottom:5px;outline: none;display: block;color: #000;font-size: 1.2rem;font-weight: 400;'.$border_bottom.'">'.($v[2] ? $v[1] : $AppUI->_($v[1])).'</span></a></td>';
			...
			$s .= '</table></td></tr>';
			$s .= '<tr><td width="100%" colspan="'.(count($this->tabs)*4 + 1).'" class="tabox" style="border:none">';
			echo $s;
