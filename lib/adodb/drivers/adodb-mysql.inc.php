	...
	function MetaForeignKeys( $table, $owner = FALSE, $upper = FALSE, $associative = FALSE )
     {
         ...
         for ( $i = 0;  $i < $num_keys;  $i ++ ) {
             $mysql_field  = explode('`, `', $matches[1][$i]);
             ...
             $num_fields = count($mysql_field);
             for ( $j = 0;  $j < $num_fields;  $j ++ ) {
                 if ( $associative ) {
                     $foreign_keys[$ref_table][$ref_field[$j]] = $mysql_field[$j];
                 } else {
                     $foreign_keys[$ref_table][] = "{$mysql_field[$j]}={$ref_field[$j]}";
                 }
             }
         }
