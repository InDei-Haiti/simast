...
class CProject extends CDpObject {
	...
	var $project_partners = NULL;
	var $project_tcoord = NULL;
	var $project_country = NULL;
	var $project_sector = NULL;
	var $project_annual_budget = NULL;
	var $project_cdonors = NULL;
	...
	function check() {
		...
		if (strlen($this->project_short_name) > 50) {
			$this->project_short_name = substr($this->project_short_name, 0, 50);
		}
		if (strlen($this->project_description) > 1000) {
			$this->project_description = substr($this->project_description, 0, 1000);
		}
	...
	function localCheck($uid,$subid){
		...
		if($uid > 0 && $subid > 0){
			...
			$q->addTable('users','u');
			$q->addWhere('p.project_id="'.$subid.'"');
			$q->addWhere('u.user_id="'.$uid.'"');
			$q->addWhere('u.user_contact=c.contact_id');
			$q->addWhere('c.contact_company=p.project_company');
			$vcom=$q->loadResult();
			if($vcom > 0){
				$res=true;
			}
	...
	function delete() {
		...
		$sql='update files set file_task="0",file_project="0" where file_project="'.$this->project_id.'"';
		db_exec($sql);
		$sql='update links set link_task="0",link_project="0" where link_project="'.$this->project_id.'"';
		db_exec($sql);
	...
	**/
	...
	/**
	 *	Overload of the dpObject::getDeniedRecords to ensure that the projects owned by denied companies are denied.
	 *
	 * @author	handco <handco@sourceforge.net>
	 * @see	dpObject::getAllowedRecords
	 * @param $uid
	...
	/* Retrieve activity sectors for a given project
	* @param int Project_id
	* @return array List of criticalTasks
	*/
	function getSectors($project_id = NULL, $limit = 1)	{
		return $this->getActField('task_sector',$project_id,$limit);
	}

	function getAreas($project_id = NULL, $limit = 1)	{
		return $this->getActField('task_areas',$project_id,$limit);
	}

	function getLocations($project_id = NULL, $limit = 1)	{
		return $this->getActField('task_locations',$project_id,$limit);
	}

	function getActField($field_name,$project_id = NULL, $limit = 1)	{
		global $limit;
	...
	*/
	function getContractors($project_id = NULL, $limit = 1)	{
		$project_id = !empty($project_id) ? $project_id : $this->project_id;
		$q = new DBQuery;
		$q->addTable('tasks');
		$q->leftJoin('projects', 'p','p.project_id = tasks.task_project');
		if ($project_id) {
			$q->addWhere('task_project = ' . $project_id);
		}
		$q->addQuery(' distinct (task_company)');
		$q->addWhere(' task_company > 0 AND task_company <> project_company');
		if($limit > 0){
			$q->setLimit($limit);
		}
	...
	*/
	...
	function store() {
		...
		if($this->project_id) {
			echo $this->project_description.'<br/>';
...
function projects_list_data($user_id = false) {
	global $AppUI, $addPwOiD, $buffer, $countryBuffer, $sectorBuffer, $sector, $sector_id, $company, $company_id,$country, $country_id, $company_prefix, $deny, $department;
	global $dept_ids, $dPconfig, $orderby, $orderdir, $projects, $tasks_critical, $tasks_problems,$progs_id,$pareas,$projtypes;
	global $tasks_sum, $tasks_summy, $tasks_total, $owner, $projectTypeId, $project_status;
	global $currentTabId,$filshow;
...
*/
	...
	$q->addTable('projects', 'p');
	$q->addQuery('p.project_id, p.project_status, p.project_color_identifier, p.project_type, p.project_name,p.project_partners,p.project_cdonors,
					p.project_description, p.project_start_date, p.project_end_date, p.project_color_identifier,
					p.project_company, p.project_status, p.project_priority, p.project_target_budget, p.project_owner,
					com.company_acronym as company_name, com.company_description,
					'./*tc.critical_task, tc.project_actual_end_date,
						if(tp.task_log_problem IS NULL, 0, tp.task_log_problem) AS task_log_problem, tt.total_tasks,
						tsy.my_tasks, ts.project_percent_complete, ts.project_duration,*/
					'u.user_username'
					);
					...
					if (isset($_REQUEST['template']) && ($_REQUEST['template'] == 'old')) {
						...
						/*if (isset($department) && $department[0] != "company_0") {
						...
						$upwhere = array();
						$wdo=false;
						$midsqlwhere='or p.project_id IN (select task_project from tasks t1 where ';

						if ($company_id &&!$addPwOiD) {
							if(is_array($company_id)){
								if(count($company_id) == 1 && $company_id[0] != "0"){
									//$q->addWhere('( p.project_company = ' . (substr($company_id[0],strlen($company_prefix))).' or p.project_id IN (select task_project from tasks t1 where t1.task_company= '.(substr($company_id[0],strlen($company_prefix))).' ) )' );
									$upwhere[]='p.project_company = ' . (substr($company_id[0],strlen($company_prefix))).' ' ;
									$lwhere[]='t1.task_company= '.(substr($company_id[0],strlen($company_prefix))).' ';
								}elseif (count($company_id) > 1){
									$wdo=true;
									$comps=array();
									foreach ($company_id as $cid){
										$tco=(int) substr($cid,strlen($company_prefix));
										if($tco > 0)$comps[]=$tco;
									}
									//$q->addWhere('(p.project_company in (' . implode(", ",$comps).') or p.project_id IN (select task_project from tasks t1  where t1.task_company IN (' . implode(", ",$comps).') ) )');
									$upwhere[]='p.project_company in (' . implode(", ",$comps).')';
									$lwhere[]='t1.task_company IN (' . implode(", ",$comps).') ';
								}
							}
						}

						if($progs_id  != '' && $progs_id[0] != 0){
							if(is_array($progs_id)){
								if(count($progs_id) == 1 && $progs_id[0] != "0"){
									$upwhere[]='p.project_donor REGEXP "[[:<:]]' . ($progs_id[0]).'[[:>:]]" ' ;
									//$lwhere[]='t1.task_com= '.(substr($company_id[0],strlen($company_prefix))).' ';
								}elseif (count($progs_id) > 1){
									$wdo=true;
									$comps=array();
									foreach ($progs_id as $cid){
										$tco=(int) $cid;
										if($tco > 0)$comps[]='( p.project_donor REGEXP "[[:<:]]' . $tco . '[[:>:]]" )';
									}

									$upwhere[]= '( '.join(" OR ",$comps) .' )';
									//$lwhere[]='t1.task_company IN (' . implode(", ",$comps).') ';
								}
							}
						}
						...
						if($pareas){	
							$lpar= array();
							foreach($pareas as $pitem){
								if($pitem > 0){
									//$q->addWhere('(t.task_areas regexp "[[:<:]]' . $pitem . '[[:>:]]" )');
									$lpar[]='t1.task_areas regexp "[[:<:]]' . $pitem . '[[:>:]]" ';
								}
							}
							if(count($lpar) > 0){
								$lwhere[] = '( '.join(" OR ",$lpar).' )';
						...
						if ($country_id) {
							if(count($country_id) == 1 && is_numeric($country_id[0]) && (int)$country_id[0] > 0){
								$dsql = " = '".$country_id[0]."'";
							...
							if($dsql !=""){
								//$q->addWhere('t.task_country '.$dsql);// = ' . $country_id);
								//$upwhere[]='p.project_company '.$dsql.' ';
								$lwhere[]='t1.task_country '.$dsql.' ';
							}
						}
						...
						if ($sector_id) {
							...
							if($dsql!=""){
								//$q->addWhere('t.task_sector '.$dsql);// = ' . $sector_id);
								$upwhere[]= 'p.project_sector '.$dsql.' ';
								$lwhere[]=	't1.task_sector '.$dsql.' ';
							}*/
							$lsec = array();
							foreach($sector_id as $sitem){
								if($sitem > 0){
									//$q->addWhere('(t.task_areas regexp "[[:<:]]' . $pitem . '[[:>:]]" )');
									$lsec[]='t1.task_sector regexp "[[:<:]]' . $sitem . '[[:>:]]" ';
								}
							}
							if(count($lsec) > 0){
								$lwhere[] = '( ' .join(" OR ",$lsec). ' )';
						...
						if ($projtypes) {
							if(count($projtypes) == 1 && (int)$projtypes[0] > 0){
								$dsql=" = '".$projtypes[0]."'";
							}elseif (count($projtypes) > 1){
								$dsql=" in (".implode(", ",$projtypes).")";
							}else {
								$dsql="";
							}
							if($dsql!=""){
								//$q->addWhere('t.task_sector '.$dsql);// = ' . $sector_id);
								$upwhere[]= 'p.project_type '.$dsql.' ';
							}
						}

						if(count($upwhere) > 0){
							$wsql = implode(' AND ',$upwhere);
						}
						if(count($lwhere) > 0){
							$wsql.=$midsqlwhere;
							$wsql.=implode(' AND ',$lwhere);
							$wsql.=') ';
						}
						if($wsql !=''){
							if(preg_match("/^or\s/i",$wsql)){
								$wsql = preg_replace('/^or\s/',"",$wsql);
							}
							$q->addWhere($wsql);
						}
					...
					// get the list of permitted companies
					$companies = arrayMerge(array('0' => $AppUI->_('All Agencies')), $companies);
					...
					$buffer = '<select name="company_id[]" title="Agencies"  class="text" multiple size=3 style="width: 200px;">';
					$buffer .= ('<option value="0" style="font-weight:bold;" '.$zstr.'>' . $AppUI->_('All Agencies')
					. '</option>'."\n");
					$company = '';
					...
					foreach ($rows as $row)
					{
						if ($row['dept_parent'] == 0)
						{
							if($company != $row['company_id'])
							{
								if($check_dep){
									//if(is_array($company_id)){
									if(@in_array($company_prefix.$row['company_id'],$company_id)){
										$ustr=$sel_str;
										if($row['company_id']!='-1'){
											$filshow=true;
										}
								...
								.$ustr
								. '>' . tr_string($row['company_name'],30) . '</option>' . "\n");
					...
					$ustr="";
					//var_dump($countryBuffer);
					$countryBuffer = '<select name="province_id[]"  class="text" multiple size=3 style="width: 200px;">'; //onChange="document.pickCountry.submit()"
					$countryBuffer .= ('<option value="0" style="font-weight:bold;" '.$zstr.'>' . $AppUI->_('All Provinces (Kenya)')
					. '</option>'."\n");
					...
					//$countryBuffer = buildProvinceList('province_id');


					//get list of all sectors
					$sectorList = dPgetSysVal('SectorType');
					...
					//var_dump($countryBuffer);
					$sectorBuffer = '<select name="sector[]" title="Sectors" class="text multiple" multiple size=3 style="width: 200px;">'; // onChange="document.pickSector.submit()
					$sectorBuffer .= ('<option value="0" style="font-weight:bold;" '.$zstr.'>' . $AppUI->_('All Sectors')
					. '</option>'."\n");

					$ustr="";
					foreach ($sectorList as $sect_id=>$sector_name)
					{
						//if ($row['dept_parent'] == 0)
						//{
						if($sector != $sect_id)
						{
							...
							$sectorBuffer .= ('<option value="' . $sect_id 	. '"'// style="font-weight:bold;"'
							//. (($sector_id == $sect_id) ? 'selected="selected"' : '')
							.$ustr
							. '>' . tr_string($sector_name,30) . '</option>' . "\n");
							$sector = $sect_id;
						}
						//}
					}
					...
					//$countryBuffer = arraySelect( $countylist, 'country_list', 'size="1" class="text"', NULL, true );
}
?>
