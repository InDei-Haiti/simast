...
<tr>
	...
	<td nowrap="nowrap">
		...
		<label for="log_pdf"><?php echo $AppUI->_('Incomplete Activities');?></label>
	</td>
...
if ($do_report) {
	...
	if ($project_id==0) { echo "<tr><th>Project Name</th><th>Activity Name</th>";} else {echo "<tr><th>Activity Name</th>";}
	echo "<th width=400>Activity Description</th>";
	echo "<th>Assigned To</th>";
	echo "<th>Activity Start Date</th>";
	echo "<th>Activity End Date</th>";
	...
	$columns = array(	
	"<b>".$AppUI->_('Activity Name')."</b>",
	"<b>".$AppUI->_('Activity Description')."</b>",
	"<b>".$AppUI->_('Assigned To')."</b>",
	"<b>".$AppUI->_('Activity Start Date')."</b>",
	"<b>".$AppUI->_('Activity End Date')."</b>",
...
if ($log_pdf) {
	// make the PDF file
		...
		$pdf->ezText("\n" . safe_utf8_decode($AppUI->_('Project Activity Report')), 12);
		if ($project_id != 0) {$pdf->ezText(safe_utf8_decode($pname), 15);}
		if ($log_all) {
			$pdf->ezText("All task entries", 9);
		} else {		
			if($end_date != ' ') {$pdf->ezText("Activity entries from ".$start_date->format($df).' to '.$end_date->format($df), 9);} else {$pdf->ezText("Activity entries from ".$start_date->format($df),9);}
		}
		$pdf->ezText("\n");
		$pdf->selectFont("$font_dir/Helvetica.afm");
		$columns = array(	
			"<b>".safe_utf8_decode($AppUI->_('Activity Name'))."</b>",
			"<b>".safe_utf8_decode($AppUI->_('Activity Description'))."</b>",
			"<b>".safe_utf8_decode($AppUI->_('Assigned To'))."</b>",
			"<b>".safe_utf8_decode($AppUI->_('Activity Start Date'))."</b>",
			"<b>".safe_utf8_decode($AppUI->_('Activity End Date'))."</b>",
