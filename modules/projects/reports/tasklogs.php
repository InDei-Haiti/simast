...
<?php
	...
	// make the PDF file
		...
		$pdf->ezText("\n" . safe_utf8_decode($AppUI->_('Activity Log Report')), 12);
		$pdf->ezText(safe_utf8_decode($pname), 15);
		if ($log_all) {
			$pdf->ezText("All activity log entries", 9);
		} else {
			$pdf->ezText("Activity log entries from ".$start_date->format($df).' to '.$end_date->format($df), 9);
		}
		$pdf->ezText("\n\n");
