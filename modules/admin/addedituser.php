...
if (!db_loadHash( $sql, $user ) && $user_id > 0) {
	...
	$titleBlock = new CTitleBlock( 'Invalid User ID', '', $m, "$m.$a" );
...
function popDept() {
    ...
    if (f.selectedIndex == 0) {
        ...
        window.open('./index.php?m=public&a=selector&dialog=1&callback=setDept&table=departments&company_id='
            ...
            + '&dept_id='+f.contact_department.value,'dept','left=50,top=50,height=250,width=400,resizable=1')
    }
}
...
<?php
	if (@$user["user_username"]){
        ...
        echo '<input type="text" class="text form-control" name="user_username" value="' . $user["user_username"] . '" maxlength="255" size="40" />';
    }
?>
	</td></tr>
...
<tr>
    ...
    <td><?php echo arraySelect($roles_arr, 'user_role', 'size="1" class="text form-control"','', true);?></td>
</tr>
<?php }
?>
<tr>
    <td align="right">* <?php echo $AppUI->_('Password');?>:</td>
    <td><input type="password" class="form-control" name="user_password" value="<?php echo $user["user_password"];?>" maxlength="32" size="32" style="width: 430px;" /> </td>
</tr>
<tr>
    <td align="right">* <?php echo $AppUI->_('Confirm Password');?>:</td>
    <td><input type="password" class="form-control" name="password_check" value="<?php echo $user["user_password"];?>" maxlength="32" size="32"  style="width: 430px;" /> </td>
</tr>
<tr>
    <td align="right">* <?php echo $AppUI->_('Name');?>:</td>
    <td><input type="text" class="form-control" name="contact_first_name" value="<?php echo $user["contact_first_name"];?>" maxlength="50"  style="width: 430px;" /> <br /><br /><input type="text" class="form-control" name="contact_last_name" value="<?php echo $user["contact_last_name"];?>" maxlength="50"  style="width: 430px;" /></td>
</tr>
<?php if ($canEdit) { ?>
<tr>
    <td align="right"> <?php echo $AppUI->_('Agency');?>:</td>
    <td>
<?php
    echo arraySelect( $companies, 'contact_company', 'class="form-control" size=1  style="width: 430px;"', $user["contact_company"] );
?>
    </td>
</tr>
<?php } ?>
<tr>
    ...
    <td>
        ...
        <input type="text" class="form-control"  style="width: 430px;" name="dept_name" value="<?php echo @$user["dept_name"];?>" size="40" disabled />
        <input type="button" class="button ce pi ahr" value="<?php echo $AppUI->_('select dept');?>..." onclick="popDept()" />
    </td>
</tr>
<tr>
    <td align="right">* <?php echo $AppUI->_('Email');?>:</td>
    <td><input type="text" class="form-control" name="contact_email" style="width: 430px;" value="<?php echo $user["contact_email"];?>" maxlength="255" size="40" /> </td>
</tr>
<tr>
    <td align="right" valign=top><?php echo $AppUI->_('Email').' '.$AppUI->_('Signature');?>:</td>
    <td><textarea class="form-control" cols=50 name="user_signature"  style="width: 430px;height: 50px;"><?php echo @$user["user_signature"];?></textarea></td>
</tr>
<tr>
...
<tr>
    <td align="left">
        <input type="button" value="<?php echo $AppUI->_('back');?>" onClick="javascript:history.back(-1);" class="button ce pi ahr" />
    </td>
    <td align="right">
    <?php if ($canEdit && !$user_id) { ?>
	...
	<input type="button" value="<?php echo $AppUI->_('submit');?>" onclick="submitIt()" class="button ce pi ahr" />
    </td>
</tr>
</table>
<?php }
