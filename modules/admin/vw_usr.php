...
?>
<div class="mtab">
<table cellpadding="2" cellspacing="1" border="0" width="100%" class="tbl tablesorter" id='tfor_sort'>
<thead>
<tr>
	...
	<?php } ?>
	<th width="150">
		<?php /*  <a href="?m=admin&a=index&orderby=user_username" class="hdr">?><?php  */ echo $AppUI->_('Login Name'); /*</a>*/?>
	</th>
	<th>
		<?php /*<a href="?m=admin&a=index&orderby=contact_last_name" class="hdr">*/ echo $AppUI->_('Real Name'); /*</a>*/?>
	</th>
	<th>
		<?php /*<a href="?m=admin&a=index&orderby=contact_company" class="hdr"><?php*/  echo $AppUI->_('Agency'); /*</a>*/?>
	</th>
</tr>
</thead>
<tbody>
...
$adminprojects = $perms->getFormPermission($AppUI->user_id, 'projects');
foreach ($users as $row) {
		if ($perms->isUserPermitted($row['user_id']) != $canLogin)
			continue;
	$testproject = false;
	if(trim($AppUI->user_type) == "1"){
		$testproject = true;
	}elseif($row['user_id']==$AppUI->user_id){
		$testproject = true;
	}elseif($row['user_create_by']==$AppUI->user_id){
		$testproject = true;
	}else{
		...
		if($row['user_type'] == "1")
			...
			foreach ($adminprojects as $id){
				if($perms->checkForm($row['user_id'], 'projects', $id, 'view')){
						$testproject = true;
				}
			}
	}
...
<?php } ?>
	...
	<!-- Moving td to ligne 114 in echo after span | was here before | Preventing closing some before opened TDs-->
	<td>
...
if ($row['contact_last_name'] && $row['contact_first_name'])
        ...
        // echo '<span style="font-style: italic">unknown</span>'; Verification de la traduction francaise
        echo '<span style="font-style: italic">'.$AppUI->_('Unknown').'</span>';
?>
	</td>
	<td>
...
<?php
$myspeed->addJs('$j(document).ready(function(){$j.tablesorter.addParser({id: "agname",is: function(s) {return false;}, format: function(s) { var str = s.toLowerCase().replace(/<[^>]*>/g,"").replace(/&nbsp;/g,""); return  str;},type: "text"}); $j("#tfor_sort").tablesorter({headers:{0:{sorter: false},2: {sorter: "agname"},3:{sorter: "agname"},4:{sorter: "agname"}}});});','code');
?>
