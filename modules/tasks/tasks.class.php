...
// user based access
$task_access = array ('0' => 'Public', '4' => 'Privileged', '2' => 'Participant', '1' => 'Protected', '3' => 'Private' );
...
$status_types = dPgetSysVal ( 'ProjectStatus' );
...
class CTask extends CDpObject {
	...
	var $task_type = NULL;
	var $task_beneficiares = NULL;
	var $task_type_of_beneficiery = Null;
	var $task_other_location2 = NULL;
	var $task_manager = NULL;

	var $task_quantity = NULL;
	var $task_units = NULL;
	var $task_programs = NULL;

	var $task_annual_budget = NULL;
	var $task_areas = NULL;
	var $task_locations = NULL;
	var $task_agency = NULL;
	//var $denied_tasks = array();
	//var $dcheck =false;
	...
	function updateDynamics($fromChildren = false) {
		...
		if ($modified_task->task_dynamic == '1') {
			...
			*/
			...
			$total_hours_allocated = ( float ) ($modified_task->task_duration * (($modified_task->task_duration_type > 1) ? dPgetConfig ( 'daily_working_hours' ) : 1));
	...
	*/
	...
	/* Retrieve activity subcontractors for a given project
	* @param int Project_id
	* @return array List of subcontractors
	*/
	function getContractors($task_id = NULL) {
		$task_id = ! empty ( $task_id ) ? $task_id : $this->task_id;
		$q = new DBQuery ( );
		$q->addTable ( 'tasks' );
		$q->addQuery ( ' distinct (task_company)' );
		if ($this->task_mode == "root") {
			$q->addWhere ( ' task_company > 0 AND task_parent="' . $task_id . '"' );
		} else {
			$q->addWhere ( ' task_company > 0 AND task_id="' . $task_id . '"' );
		}
	...
	function delete($fkill = 0) {
		...
		if (( int ) $fkill == 1) {
			$q->addTable ( "files" );
			$q->addWhere ( 'file_task="' . $task->task_id . '"' );
			$q->addquery ( "file_id" );
			$flist = $q->loadHashList ();

			if ($flist) {
				foreach ( $flist as $fil => $bvc ) {
					$tfile = new CFile ( );
					$tfile->load ( $fil );
					$tfile->delete ();
					unset ( $tfile );
			...
			$q->setDelete ( 'links' );
			if (! empty ( $childrenlist )) {
				$q->addWhere ( 'link_task IN (' . implode ( ', ', $childrenlist ) . ', ' . $task->task_id . ')' );
			} else {
				$q->addWhere ( 'link_task=' . $task->task_id );
			}
			if (! ($q->exec ())) {
				return db_error ();
			}
			...
			$sql='update files set file_task="0" where file_task="'.$task->task_id.'"';
			db_exec($sql);
			$sql='update links set link_task="0" where link_task="'.$task->task_id.'"';
			db_exec($sql);
		}
		
		$q->setDelete('permission_form');
		$q->addWhere('module ="activity"');
		$q->addWhere('form =' . $this->task_id);
		$q->exec();
		$q->clear();
	...
	function email_log(&$log, $assignees, $task_contacts, $project_contacts, $others, $extras) {
		...
		if ($this->task_parent != $this->task_id) {
			...
			$req = & $q->exec ( QUERY_STYLE_NUM );
			if ($req) {
				$body .= $AppUI->_ ( 'Parent Activity', UI_OUTPUT_RAW ) . ': ' . htmlspecialchars_decode ( $req->fields [0] ) . "\n";
			}
			$q->clear ();
		}
		$body .= $AppUI->_ ( 'Activity', UI_OUTPUT_RAW ) . ': ' . $this->task_name . "\n";
		$task_types = dPgetSysVal ( 'TaskType' );
		$body .= $AppUI->_ ( 'Activity Type', UI_OUTPUT_RAW ) . ':' . $task_types [$this->task_type] . "\n";
	...
	function getTasksForPeriod($start_date, $end_date, $company_id = 0, $user_id = null, $filter_proj_archived = false, $filter_proj_completed = false) {
		...
		/*$cev=(int)$ev_filter;
		$cevs="$cev";
		if(is_int($cev) && $ev_filter >= 0 && $cevs == $ev_filter ) {
		$q->addWhere('event_type = "'.$ev_filter.'"');
		}*/
		if ($user_id) {
	...
	function canAccess($user_id) {
		...
		if(!$_SESSION['dtcheck']){
			$_SESSION['denied_tasks'] = $this->getDeniedRecords ( $user_id );	
			$_SESSION['dtcheck'] = true;
		}else{
			$denied_tasks = $_SESSION['denied_tasks'];
		}
		...
		/*
		* TODO change this in case when real task access data will be used
		* for a while we'll use this like for privileged by default
		*/
		//$otacc=$this->task_access;
		//$this->task_access = 4;
		...
		switch ($this->task_access) {
			case 0 :
				...
				if(!$_SESSION['dpcheck']){
					...
					$_SESSION['denied_projects']=$denied_projects;
					$_SESSION['dpcheck']=true;
				}else{
					$denied_projects=$_SESSION['denied_projects'];
				}
	...
	*/
	 ...
	 * @param null $users
	 *
	 * @internal param \hash $string a hash for the returned hashList
	 *
	 * @internal param \users $array an array of user_ids calculating their assignment capacity
	...
	function remind($module, $type, $id, $owner, &$args) {
		...
		$prefix = $AppUI->_ ( 'Activity Due', UI_OUTPUT_RAW );
		if ($diff == 0) {
			...
			$prefix = $AppUI->_ ( 'Activity', UI_OUTPUT_RAW );
		} else {
	...
	function getUrl($urlType = 'view', $taskType = NULL) {
		/* if ($taskType == NULL)
		 $taskType = $this->task_type_of_beneficiery;
	
			$modules = dPgetSysVal ( 'ClientModules' );
			$unit = $modules [$companyType]; */
		$url_array = array ("view" => "./index.php?m=tasks&a=view&task_id=$this->task_id", "add" => "./index.php?m=tasks&&a=addedit", "edit" => "./index.php?m=tasks&a=addedit&task_id=$this->task_id" );
...
function showtask(&$a, $level = 0, $is_opened = true, $today_view = false, $hideOpenCloseLink = false, $allowRepeat = false, $project_class = "") {
	global $AppUI, $done, $query_string, $durnTypes, $userAlloc, $showEditCheckbox;
	global $tasks_opened, $tasks_closed, $user_id;
	global $top_row, $pbox_class, $project_id,$m,$adwn,$adm_rights;
	global 	$sector_list,	$activity_types ,$status_types,$companies;
	...
	$tcan_edit = $task_obj->localcheck($a ['task_id']);

	require_once $AppUI->getModuleClass ( 'projects' );
	
	$pobj= new CProject();
	$pcan_edit = $pobj->localCheck($AppUI->user_id,$a['task_project']);
	...
	//if ($level == 0) {
	if($a['kids'] > 0){
		$tr_class = "rowp_" . $a ['task_id'];
		$top_row = "rowc_" . $a ['task_id'];
		$td_html = '<span id="fon_' . $a ['task_id'] . '" kids="'.$a['kids'].'" load="not"></span>';
	} elseif ($level > 0) {
		$tr_class = $top_row;
		$td_html = "";
	}
	$s = "\n<tr class='$tr_class '>"; //$pbox_class
	// edit icon
	$s .= "\n\t<td>";
	$canEdit = getPermission ( 'tasks', 'edit', $a ['task_id'] );
	$canViewLog = $perms->checkModuleItem ( 'task_log', 'view', $a ['task_id'] );
	if ($canEdit && $a ['task_mode'] == "root" && ($tcan_edit || $adm_rights || $pcan_edit)) {
		$s .= ("\n\t\t" . '<a href="?m=tasks&a=addedit&task_id=' . $a ['task_id'] . '">' . "\n\t\t\t" . '<img src="./images/icons/pencil.gif" alt="' . $AppUI->_ ( 'Edit Activity' ) . '" border="0" width="12" height="12">' . "\n\t\t</a>");
	}
	$s .= "\n\t</td>";
	...
	/*if ($a ['task_mode'] == "root") {*/
	$imgu = './images/corner-dots.gif';
	/*} else {
	$imgu = "./images/sub_activity.gif";
	}*/
	for($y = 0; $y < $level; $y ++) {
		$s .= ('<img src="' . (($y + 1 == $level) ? $imgu : './images/shim.gif') . '" width="16" height="12" border="0">');
	}
	if ($level == 0 && $a ['task_mode'] == "sub") {
		$s .= ('<img src="' . $imgu . '" width="16" height="12" border="0">');
	}
	...
	if ($a ['task_milestone'] > 0) {
		$s .= ('&nbsp;<a href="./index.php?m=tasks&a=view&task_id=' . $a ['task_id'] . '" ' . $alt . '>' . '<b>' . $a ['task_name'] . '</b></a>' . '<img src="./images/icons/milestone.gif" border="0"></td>');
	} else if ($a ['task_dynamic'] == 1) {
		if (! ($today_view || $hideOpenCloseLink)) {
			$s .= ('<a href="index.php' . $query_string . (($is_opened) ? ('&close_task_id=' . $a ['task_id'] . '"><img src="images/icons/collapse.gif" align="center"') : ('&open_task_id=' . $a ['task_id'] . '"><img src="images/icons/expand.gif"')) . ' border="0" /></a>');
		}
		$s .= ('&nbsp;<a href="./index.php?m=tasks&a=view&task_id=' . $a ['task_id'] . '" ' . $alt . '><b><i>' . $a ['task_name'] . '</i></b></a></td>');
	} else {
		$s .= ('&nbsp;<a href="./index.php?m=tasks&a=view&task_id=' . $a ['task_id'] . '" ' . $alt . '>' . $a ['task_name'] . '</a></td>');
	}
	//sector
	if (!($m == 'tasks' && $adwn == 'view') && $m != 'projects'){
	
		//$s .= '<td >' . $activity_types [$a ['task_type']] . '</td>';
	
		$s .= '<td >' . multiView( $sector_list,$a ['task_sector']) . '</td>';
		/*if ($project_id == 0) {
			$s .= "<td>" . $a ['project_name'] . "444</td>";
		}*/
	}
	/*if (! $_SESSION ['companies']) {
		$q = new DBQuery ( );
	...
	$cur_podr = $task_obj->getContractors ( $a ['task_id'] );
*/
	/*$podr = "";
	if (is_array ( $cur_podr ) && count ( $cur_podr ) > 0) {
		foreach ( $companies as $cname => $cid ) {
			if (array_key_exists ( $cid, $cur_podr )) {
				$podr .= "&nbsp;" . $cname . ",";
			}
		}
	}*/
	...
	//$podr = preg_replace ( "/\,$/", "", $podr );

	if($m != 'companies'){
		$podr = multiView($companies,$task_obj->task_agency);
		$s .= '<td >' . $podr . '</td>';
	}

	$area_list = '';
	if($task_obj->task_areas != ''){
		$q= new DBQuery();
		$q->addTable('st_area');
		$q->addWhere('id IN ('.$task_obj->task_areas.' )');
		$q->addQuery('concat(prex," ",title) as name');
		$area_list = @join(", ",array_keys(@$q->loadArrayList()));
	}

	//if($m != 'companies'){
		$s.= '<td class="moreless">'.$area_list.'</td>';
	//}


	$loc_list ='';
	if($task_obj->task_locations != ''){
		$q  =new DBQuery();
		$q->addTable('administrative_regions');
		$q->addWhere('region_id  in ('.$task_obj->task_locations .')');
		$q->addQuery('region_name');
		$loc_list = @join(", ",array_keys(@$q->loadArrayList()));
	}
	$s.= '<td class="moreless">'.$loc_list.'</td>';


	// duration or milestone
	unset ( $style );
	$s .= ('<td  align="center" style="' . $style . '">' . ($start_date ? $start_date->format ( $df ) : '-') . '</td>' . // . '<td align="center" nowrap="nowrap" style="' . $style . '">' . $a['task_duration']
	//. ' ' . $AppUI->_($durnTypes[$a['task_duration_type']]) . '</td>'
	'<td  align="center" style="' . $style . '">' . ($end_date ? $end_date->format ( $df ) : '-') . '</td>');
	if ($today_view) {
		$s .= ('<td  align="center" style="' . $style . '">' . $a ['task_due_in'] . '</td>');
	...
	$s .= '<td>' . $status_types [$a ['task_status']] . '</td>';
	//$s .= ("\n\t" . '<td align="right">' . intval($a['task_percent_complete']) . '%</td>'
	//	   . "\n\t");
	if ($m == 'tasks' && $adwn == 'view') {
		$s.='<td>'.$a['task_description'].'</td>';
	}
	$s .= '</tr>';
	echo $s;
}
...
function findchild(&$tarr, $parent, $level = 0) {
	global $tasks_opened, $tasks_closed, $tasks_filtered, $children_of, $pbox_class;
	$tasks_closed = (($tasks_closed) ? $tasks_closed : array ());
	$tasks_opened = (($tasks_opened) ? $tasks_opened : array ());
	...
	foreach ( $tarr as $x => $task ) {
		if ($task ['task_parent'] == $parent && $task ['task_parent'] != $task ['task_id']) {
			...
			showtask ( $task, $level, $is_opened, false, $no_children, $pbox_class );
...
function sort_by_item_title($title, $item_name, $item_type) {
	...
	if (isset ( $item_order )) {
		echo ('<img src="./images/arrow-' . (($item_order == SORT_ASC) ? 'up' : 'down') . '.gif" width="11" height="11">');
	} else {
...
function  updateProjectTotals($projectId){
	if($projectId > 0){
		$sql = 'select task_target_budget, task_annual_budget, task_beneficiares from tasks where task_project="'.(int)$projectId.'"';
		$tres = mysql_query($sql);
		$total=array('budget'=>0,'beneficiares'=>0,'annual_budget'=>'');
		$annuals = array();
		if(mysql_num_rows($tres) > 0){
			while($ctact = mysql_fetch_assoc($tres)){				
				$total['budget']+=(int)$ctact['task_target_budget'];
				$total['beneficiares']+=(int)$ctact['task_beneficiares'];
				if($ctact['task_annual_budget'] != ''){
					$parts = explode(";",$ctact['task_annual_budget']);
					foreach ($parts as $pt){
						$sit = explode("|",$pt);
						if(!isset($annuals[$sit[0]])){
							$annuals[$sit[0]] = 0;
						}
						$annuals[$sit[0]]+=(int)$sit[1];
					}
				}
			}
			$newanns = array();
			foreach ($annuals as $year => $ybud) {
				$newanns[]=$year.'|'.$ybud;
			}
			if(count($annuals) > 0){
				$total['annual_budget']=join(";",$newanns);
			}
		
			$sql='update projects set project_target_budget="'.$total['budget'].'",
									  project_actual_budget = "'.$total['beneficiares'].'",
									  project_annual_budget = "'.$total['annual_budget'].'"
