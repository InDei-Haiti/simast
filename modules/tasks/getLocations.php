...
$district_string = dPgetParam( $_REQUEST, 'district_id', 0 );
$district_array = explode("," , $district_string);
$district_id = $district_array[0];
//fetch admin level 1
$q = new DBQuery();
$q->addTable('administrative_regions');
$q->addQuery('region_id, region_name');
$q->addWhere('region_parent in (' . $district_id  . ')');
$q->addOrder('region_parent, region_name');
$admin4_list = $q->loadArray();

$encoded = json_encode($admin4_list);
echo $encoded;
