...
$join = '';
// pull tasks
$select = ('distinct tasks.task_id, task_parent, task_name, task_sector, task_type, task_country, task_admin2, task_admin3, '
		   . 'task_admin4, task_other_location, task_gps_x, task_gps_y,task_start_date, task_end_date, ' 
...
//filter by sector
if ( $sector > -1) {
	$where .= ' AND tasks.task_sector = ' . intval($sector);
}

//filter by projects
if ($selected_project_id ) {
	$where .= ' AND projects.project_id = ' . intval($selected_project_id);
}
...
function chAssignment(project_id, rmUser, del) {
	...
	if (del == true) {
		if (c == 0) {
			alert ('<?php echo $AppUI->_('Please select at least one Activity!', UI_OUTPUT_JS); ?>');
		} 
		else if (a == 0 && rmUser == 1){
			alert ('<?php echo $AppUI->_('Please select at least one Assignee!', UI_OUTPUT_JS); ?>');
		} 
		else if (confirm('<?php echo $AppUI->_('Are you sure you want to unassign the User from Activity(ies)?', UI_OUTPUT_JS); ?>')) {
			f.del.value = 1;
			f.rm.value = rmUser;
...
<tr>
  <td align='right'>
	...
	<tr>
	  ...
	  <td><label for="show_incomplete"><?php echo $AppUI->_('Incomplete Activities Only'); ?></label></td>
	</tr>
	</table>
	-->
  </td>
...
<tr>
  <th width="10">&nbsp;</th>
  <th width="140"><?php sort_by_item_title('Activity Name', 'task_name', SORT_STRING);?></th>
  <th nowrap="nowrap"><?php sort_by_item_title('Sector', 'activity_sector', SORT_STRING);?></th>
  <th nowrap="nowrap"><?php echo $AppUI->_('Activity Type')?></th>
  <th nowrap="nowrap"><?php echo $AppUI->_('Country')?></th>
  <th nowrap="nowrap"><?php echo $AppUI->_('Admin1')?></th>
  <th nowrap="nowrap"><?php echo $AppUI->_('Admin2')?></th>
  <th nowrap="nowrap"><?php echo $AppUI->_('Admin3')?></th>
...
<tr>
  <td colspan="<?php echo $cols; ?>" align="right">
	...
	<a href="<?php echo 'index.php'.$query_string.'&close_task_all=1';?>"><?php echo $AppUI->_('Close All Activities'); ?></a>
...
<tr>
  ...
  <td>=<?php echo $AppUI->_('Future Activity'); ?>&nbsp;&nbsp;</td>
...
echo $AppUI->_('Close All Activities'); ?></a> 
	<?php } 
?>
	</td>
</tr>
