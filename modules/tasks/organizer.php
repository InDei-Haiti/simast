...
function fixate_task($task_index, $time, $dep_on_task) {
	...
	foreach($tasks as $task2) {
		...
		if($task2["fixed"] && (
			...
			if(!$option_advance_if_possible || $task2["task_percent_complete"] != 100) {
				...
				if($vital) {
					log_info("Activity can't be set to [" . formatTime($start_date) . " - ". formatTime($end_date) . "] due to conflicts with task " . task_link($task2) . ".");
					// OBS: I'm asuming the dependent task will start next day
					fixate_task($task_index, $t2_end + SECONDS_PER_DAY, $dep_on_task);
					return;					
				} else {
					log_info("Activity conflicts with task " . task_link($task2) . " but there are no vital users.");
				}
			} else {
				log_info("Activity " . task_link($task2) . " is complete, I won't check if it is overllaping");
			}
		}
	}
	...
	if($do == "ask") {
		...
		log_action("Activity " . task_link($tasks[$task_index]) . " fixated to " . formatTime($start_date) );
...
<tr>
	...
	<td nowrap><h1><?php echo $AppUI->_('Activities Organizer Wizard'); ?></h1></td>
...
if($do != "conf") {
	...
	while ($row = mysql_fetch_array( $dtrc, MYSQL_ASSOC )) {
		...
		if(!$row["task_duration"] && $row["task_end_date"] == $NO_DATE ) {
			log_error("Activity " .task_link($row) . " has no duration.",
		...
		if(!$row["task_dynamic"] && $row["task_end_date"] == $NO_DATE ) {
			...
			if($do=="ask" && $option_no_end_date_warning) {
				log_warning("Activity " . task_link($row) . " has no end date. Using tasks duration instead.",
			...
			if(!$row["task_dynamic"] && $row["task_percent_complete"] == 0) {
				...
				if($end_time < time()) {
					if($option_check_delayed_tasks) {
						log_warning("Activity " .task_link($row) . " started on " . $row["task_start_date"] . " and ended on " . formatTime($end_time) . "." ,
	...
	if(!$action) {
		echo "<font size=2><strong>".$AppUI->_('Activities are already organized')."</strong></font><br />";
	}
...
if ($do=="conf" || $action) {
	if(!$errors) {
		...
		if($do == "ask") {
			...
			echo "<font size=2><strong>".$AppUI->_('Activities has been reorganized')."</strong></font><br />";
		} else if ($do == "conf") {
