...
for($i=0; $i <= $sizeof; $i++) {
	...
	if ($_POST['task_id'] > 0) {
		...
		if ($rm && $del){
			...
			if ($overAssignment) {
				$AppUI->setMsg('Some Users could not be unassigned from Activity', UI_MSG_ERROR);
			} else {
			...
			if (($msg = $obj->removeAssigned($user_id))) {
				...
				$AppUI->setMsg('User unassigned from Activity', UI_MSG_OK);
			}
		}
		if (isset($hassign) && ! $del == 1) {
			...
			if ($overAssignment) {
				...
				$AppUI->setMsg('User(s) assigned to Activity', UI_MSG_OK);
			}
		}
		// process the user specific task priority
		if ($chUTP == 1) {
			$obj->updateUserSpecificTaskPriority($user_task_priority, $user_id);
			$AppUI->setMsg('User specific Activity Priority updated', UI_MSG_OK, true);
		}
		...
		if ($store == 1) {
			if (($msg = $obj->store())) {
				...
				$AppUI->setMsg('Activity(ies) updated', UI_MSG_OK, true);
			}
		}
	}
}
if ($rm && $del) {
	$AppUI->setMsg('User(s) unassigned from Activity', UI_MSG_OK);
}
$AppUI->redirect();
?>
