...
function setMilestoneEndDate(checked){
    if(checked){
        document.editFrm.end_date.value      = document.editFrm.start_date.value;
        document.editFrm.task_end_date.value = document.editFrm.task_start_date.value;
    } 
}
...
function popCalendar(field){
	...
	task_cal = document.getElementById(field);
	idate = task_cal.value;
	window.open('index.php?m=public&a=calendar&dialog=1&callback=setCalendar&date=' + idate, 'calwin', 'top=250,left=250,width=251, height=220, scrollbars=no, status=no,resizable');
}
...
function setCalendar(idate, fdate) {
	...
	//fld_date = document.getElementByName(calendarField);
	$j("input.[name='"+calendarField+"']").val(idate);
	//calendarField.value = fdate;
	tsc=calendarField.replace("task","");
	tsc=tsc.replace("__","_");
	tsc=tsc.replace(/^_/,"");
	//fld_date.value = idate;
	$j("input.[name='"+tsc+"']").val(fdate);
	var adds=calendarField.match(/sub_\d{1,}_/);
	if(adds){
		if(adds[1]){
			tsc=adds[0];
		}
	...
	e_date = document.getElementById(tsc+'task_' + 'end_date');
	e_fdate = document.getElementById(tsc+'end_date');
	if (calendarField.match('start_date')) {
...
function calcFinish(f) {
	//var int_st_date = new String(f.task_start_date.value);
	var int_st_date_time = new String(f.task_start_date.value);	
