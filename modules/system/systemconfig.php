	...
	if(!strstr($c['config_name'],'regular_')){
		...
		switch ($c['config_type']) {
			case 'select':
				...
				$entry = "<select class='form-control' name='dPcfg[{$c['config_name']}]'>\n";
...
<table cellspacing="0" cellpadding="3" border="0" class="std" width="100%" align="center">
	...
	?>
	<tr>
 		<td align="right" colspan="2"><input class="button ce pi ahr" type="submit" name="do_save_cfg" value="<?php echo $AppUI->_('Save');?>" /></td>
	</tr>
</table></form>
...
function clear() {
    $("#idspecialite").each(function () { //added a each loop here
        $(this).select2('val', '')
    });
    $("#iddiplometype").val("");
    $("#idcompagnie").val("");
    $("#annee_experience").val("");
    $("#ordre").val("");
    $('#profileupdated').attr('checked', false);
    //
}
</script>
<?php echo "</div>";?>