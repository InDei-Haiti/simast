...
function showRow($id=0, $key=0, $title='', $value='') {
	...
	// edit form
		...
		$s .= '<td>&nbsp;</td>';
		$s .= '<td valign="top">'.arraySelect( $keys, 'sysval_key_id', 'size="1" class="form-control"', $key).'</td>';
		$s .= '<td valign="top"><input type="text" name="sysval_title" value="'.dPformSafe($title).'" class="form-control" /></td>';
		$s .= '<td valign="top"><textarea name="sysval_value" class="form-control" rows="5" cols="40">'.dPformSafe($value).'</textarea></td>';
		$s .= '<td><input type="submit" value="'.$AppUI->_($id ? 'edit' : 'add').'" class="button ce pi ahr" /></td>';
		$s .= '<td>&nbsp;</td>';
	} else {
