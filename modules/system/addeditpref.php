...
<?php
	...
	echo arraySelect( $langlist, 'pref_name[LOCALE]', 'class=form-control size=1', @$prefs['LOCALE'], true );
...
<?php
	...
	echo arraySelect( $tabview, 'pref_name[TABVIEW]', 'class=form-control size=1', @$prefs['TABVIEW'], true );
?>
	</td>
</tr>
...
<?php
    ...
    $f = "%d %B %Y"; $dates[$f] = $ex->format( $f );
    $f = "%B %d, %Y"; $dates[$f] = $ex->format( $f );
   echo @$prefs['SHDATEFORMAT'];
	echo arraySelect( $dates, 'pref_name[SHDATEFORMAT]', 'class=form-control size=1', @$prefs['SHDATEFORMAT'], false );
?>
	</td>
</tr>
...
<?php
	...
	echo arraySelect( $times, 'pref_name[TIMEFORMAT]', 'class=form-control size=1', @$prefs['TIMEFORMAT'], false );
?>
	</td>
</tr>
...
<?php
	...
	echo arraySelect( $currencies, 'pref_name[CURRENCYFORM]', 'class=form-control size=1', @$prefs['CURRENCYFORM'], false );
?>
	</td>
</tr>
...
<?php
	...
	$temp = $AppUI->setWarning( false );
	echo arraySelect( $styles, 'pref_name[UISTYLE]', 'class=form-control size=1', $uis, true , true);
	$AppUI->setWarning( $temp );
?>
	</td>
</tr>
...
<?php
	...
	echo arraySelect( $taskAssMax, 'pref_name[TASKASSIGNMAX]', 'class=form-control size=1', $tam, false );
...
<?php
	...
	echo arraySelect( $event_filter_list, 'pref_name[EVENTFILTER]', 'class=form-control size=1', @$prefs['EVENTFILTER'], true);
?>
	</td>
</tr>
<tr>
...
<?php
	...
	echo arraySelect( $notify_filter, 'pref_name[MAILALL]', 'class=form-control size=1', @$prefs['MAILALL'], true);
...
<tr>
	<td align="left"><input class="button ce pi ahr"  type="button" value="<?php echo $AppUI->_('back');?>" onClick="javascript:history.back(-1);" /></td>
	<td align="right"><input class="button ce pi ahr" type="button" value="<?php echo $AppUI->_('submit');?>" onClick="submitIt()" /></td>
</tr>
</table>
</div>
<?php echo "</div>";?>