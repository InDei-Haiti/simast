...
$titleBlock->addCell(
	arraySelect( $modules, 'module', 'size="1" class="form-control" onchange="document.modlang.submit();"', $module )
);
echo "&nbsp;";echo "&nbsp;";echo "&nbsp;";
$titleBlock->addCell(
	$AppUI->_( 'Language' )
...
$titleBlock->addCell(
	arraySelect( $locales, 'lang', 'size="1" class="form-control" onchange="document.modlang.submit();"', $lang, true ), '',
	'', '</form>'
);
...
if ($lang == 'en') {
	echo "<tr>\n";
	echo "<td><input type=\"text\" name=\"trans[$index][abbrev]\" value=\"\" size=\"20\" class=\"form-control\" /></td>\n";
	echo "<td><input type=\"text\" name=\"trans[$index][english]\" value=\"\" size=\"40\" class=\"form-control\" /></td>\n";
...
<tr>
	<td><?php
		if ($k != @$langs['english']) {
			$k = dPformSafe( $k, true );
			if ($lang == 'en') {
				echo "<input type=\"text\" name=\"trans[$index][abbrev]\" value=\"$k\" size=\"20\" class=\"form-control\" />";
			} else {
				echo $k;
			}
		} else {
	...
	<td><?php
		...
		if ($lang == 'en') {
			if (strlen($langs['english']) < 40) {
				echo "<input type=\"text\" name=\"trans[$index][english]\" value=\"{$langs['english']}\" size=\"40\" class=\"form-control\" />";
			} else {
			  $rows = round(strlen($langs['english']/35)) +1 ;
			  echo "<textarea name=\"trans[$index][english]\"  cols=\"40\" class=\"form-control\" rows=\"$rows\">".$langs['english']."</textarea>";
			}
		} else {
			echo $langs['english'];
	...
	<td><?php
		if ($lang != 'en') {
			...
			if (strlen($langs['lang']) < 40) {
				echo "<input type=\"text\" name=\"trans[$index][lang]\" value=\"{$langs['lang']}\" size=\"40\" class=\"form-control\" />";
			} else {
			  $rows = round(strlen($langs['lang']/35)) +1 ;
			  echo "<textarea name=\"trans[$index][lang]\"  cols=\"40\" class=\"form-control\" rows=\"$rows\">".$langs['lang']."</textarea>";
			}
		}
	?></td>
...
<tr>
	<td colspan="4" align="right">
		<input type="submit" value="<?php echo $AppUI->_( 'submit' );?>" class="button ce pi ahr" />
	</td>
</tr>
</form>
</table>
