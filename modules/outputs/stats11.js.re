...
sFrames.prototype.run = function(){
	...
	if((areas.col.length + areas.row.length) == 0 ){
		console.log((areas.col.length + areas.row.length));
		$j(".stab_let").attr("disabled",true);			
		return false;		
	}
	...
	if (!wrong) {
		...
		$j.ajax({
			type: "post",
			data: 'mode=btable&calcs=' + JSON.stringify(data)+'&calcs2=' + JSON.stringify(self.collector()),
...
var grapher = (function(my){
	...
	var bars = function(categoriesv,seriesv){
		$j('#graph_home').highcharts({
			chart: {
				type: 'bar'
			},
			title: {
	...
	return {
		...
		build: function(same){
			...
			if (pgData[0].length > 0) {
				...
				//console.log($('#tthome').find("table:last tr td:last").html());
				var options = null;
				var data = null;
				//var chart = null;
				if(pgData[1]['cmode']=='bars' || pgData[1]['cmode']=='pbars' || pgData[1]['cmode']=='sbars' ){
					...
					for(idx=0;idx<series.length;idx++){
						part = [];
						part.push(series[idx].name);
						for(idy=0;idy<series[idx].data.length;idy++){
							part.push(series[idx].data[idy]);
						}
						listall.push(part);
					...
					//chart = new google.visualization.Bar(document.getElementById('graph_home'));
					//chart = new google.charts.Bar(document.getElementById('graph_home'));
					...
					for(idx=0;idx<series.length;idx++) {
						if(pieChoke) {
							if (parseInt($('#pieChoke').val()) == idx) {
								data.addColumn('number', series[idx].name);
							}
						}else {
							data.addColumn('number', series[idx].name);
						}
					...
					chart = new google.visualization.LineChart(document.getElementById('graph_home'));
					...
					if($j("#sperc-rows").is(":checked") || $j("#sperc-cols").is(":checked")){
                        data.addColumn('decimal', 'Percentage');
					}else{
					...
					console.log(listall);
					chart = new google.visualization.PieChart(document.getElementById('graph_home'));
