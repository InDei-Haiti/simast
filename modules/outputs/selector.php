...
require_once($AppUI->getSystemClass("genericTable"));	
	...
	/* if(isset($_GET['wform_'.$fuid])){
		...
		foreach ($_GET['wform_'.$fuid] as $fld => $valfld){
			...
			if($valfld['value'] && $fld.$valfld['operator']){
				$filter .= $and;
				$filter .= " ".'wform_'.$fuid.'.'.$fld.$valfld['operator']."'".$valfld['value']."'";
			}
		}
	...
	if($data_id && count($data_id)>0){
		...
		if(count($clients)){
			...
			foreach ($clients as $cid){
				...
				echo 'date: '.$date_entry."',task:".$task_id.",form".$fuid.",clints".$cid;
				if(!$res){
					$sql = "INSERT INTO `beneficieries`(`date_entry`, `task_id`, `form_id`, `registry_id`) VALUES ('".$date_entry."',".$task_id.",".$fuid.",".$cid.")";
					;
					if(!db_exec($sql)){
						$counterror += 1;
					}else{
			...
			if($counterror){
				$AppUI->setMsg ( 'You have error on '.$counterror, UI_MSG_ERROR );
			}else{
...
{
	var URL = 'index.php?m=tasks&a=selector&&task_id=<?php echo $task_id?>';
...
if($filter){
	$count = mysql_query('SELECT count(*) as count FROM  ' . 'wform_'.$fuid.' WHERE id NOT IN (SELECT registry_id FROM beneficieries WHERE task_id='.$task_id.' AND form_id='.$fuid.') AND '.$filter);
}else{
	$count = mysql_query('SELECT count(*) as count FROM  ' . 'wform_'.$fuid.' WHERE id NOT IN (SELECT registry_id FROM beneficieries WHERE task_id='.$task_id.' AND form_id='.$fuid.')');
}
...
$q->addTable('wform_'.$fuid);
if($filter){
	$q->addWhere('id NOT IN (SELECT registry_id FROM beneficieries WHERE task_id='.$task_id.' AND form_id='.$fuid.') AND '.$filter);
}else{
...
function getSysval($array,$field,$val){
	...
	if (count($array) > 0) {
		foreach ($array as $nitem) {
			if($nitem['fld']===$field){
				$psv = $nitem["raw"]["sysv"];
				if($psv==='SysDepartment' || $psv==='SysCommunes' || $psv==='SysCommunalSection'){
...
<!-- <form action="index.php?m=public&a=client_selector&dialog=1&<?php if(!is_null($call_back)) echo 'call_back='.$call_back.'&'; ?>task_id=<?php echo $task__id.$partStr ?>" method='post' name='frmClientSelect'>
 -->
<form action="index.php?m=tasks&a=selector&task_id=<?php echo $task_id ?>" method='post' name='frmClientSelect' id="formSelect">
...
<h1 style="margin-left:5px;font-size: 16px;font-weight: bold;font-family: Osaka,verdana,Sans-Serif;olor: #08245b;margin-bottom: 3px;margin-top: 5px">
<?php echo $AppUI->_('Add Beneficiaries For '); ?> <?php echo $activity_name ?></h1><?php 
echo '<strong style="margin-left:5px;"><!--<a href="/index.php?'.$AppUI->getState('SAVEDPLACE').'">'.$AppUI->_('Back').'</a>-->
		<a href="/index.php?m=tasks&a=view&task_id='.$task_id.'">'.$AppUI->_('view activity').'</a> :
...
<?php
	...
	foreach($clients as $cliid => $client_data)
	{
		...
		$row_data = array();
		$checked = "";
		$checked = in_array($cliid, $clients_id) ? 'checked="checked"' : '';
...
foreach ($formsFlds as $index => $field){
	...
	if($field['type']==='numeric'){
		 ...
		 */
		$select = '<input type="date" id="'.$index.'_value" name="'.'wform_'.$fuid.'['.$index.'][value][]" value="'.$_GET['wform_'.$fuid][$index]['value'].'"/> ';
		$function = '<select  id="'.$index.'_operator" name="'.'wform_'.$fuid.'['.$index.'][operator]"><option value="=">is</option><option  value="<="><=</option><option value=">=">>=</option></select>';
...
?>
<div align="left" style="background:white;margin-top:2px;padding: 10px">
<fieldset id="filters" style="margin-left: -7px;"
	class="collapsible collapsed header_collapsible collapsed">
	<legend
	   ...
	   <table id="filterstab" style="display: none; width: 100%">
			<tbody>
				<?php 
...
<fieldset id="options" style="margin-left: -7px;"
	...
	<legend
	   ...
	   <table id="optionstab" style="display: none; width: 100%">
			<tbody>
				<?php 
				...
				<tr>
					<td style="width:1%;margin: 0;padding: 0;border: 0;outline: 0;font-size: 100%;vertical-align: baseline;background: transparent;">
						...
						<br/>
						<select id="available_columns" multiple="multiple" size="10" style="width:150px">
							<?php 
							if(count($selected_columns)>0){
								foreach ($formsFlds as $index => $field){
					...
					<td style="width:1%;margin: 0;padding: 0;border: 0;outline: 0;font-size: 100%;vertical-align: baseline;background: transparent;">
						...
						<br/>
						<select id="selected_columns" multiple="multiple" name="selected_columns[]" size="10" style="width:150px">
							<?php 
							//var_dump($digest);
...
$link = '\'/index.php?m=tasks&a=selector&&task_id=\'+'.$task_id;
/*$searchscpt = '';*/
...
<table style="margin-left:-3px;">
	<tr>
		<td><?php echo $prev;?></td><td><?php echo $next;?></td><td><div style="margin-top:35px;"><?php echo $info;?></div></td>
	</tr>
</table>
