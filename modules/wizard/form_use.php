...
if ($fuid > 0) {
	...
	if ($_GET['todo'] === 'save') {
		...
		//if($parent_id)
			//$wz->parent = $parent_id;
		$ref = $_POST['ref'];
		if($wz->multiplicity()!="ManyToMany" && $wz->parent && !$ref){
			echo 'Error system can\'t save';
			exit;
		}
		if(!$ref)
			$ref = 0;
	...
	$wz->tableWrap();
	$blist = '';
    echo "<br/><div class='card' style='overflow: auto'>";
	if ($xmode != 'view') {
		...
		if($teaser === false){
			...
			$titleBlock->addCrumb("?m=projects&a=view&project_id=".$project_id."&tab=2", $AppUI->_('view project ewr'));
			$titleBlock->addCrumb("?m=projects", $AppUI->_('list projects'));
            echo '<div class="block-header"><h2 style="border-bottom: 1px solid #d0d0d0;padding-bottom: 10px">';
			$titleBlock->show();
			echo '</h2></div>';
		}
		$ref = "";
		...
		$blist .= '<table cellspacing="1" cellpadding="1" border="0" class="std">
					<tbody><tr>
						<td valign="top">
						<table>';
	} else {
	...
	if ($xmode === 'view' && $useID > 0) {
		diskFile::init ();
		$titleBlock = new CTitleBlock('Details on <a href="?m=wizard&a=form_use&fid='.$fuid.'&idIns='.$idIns.'&todo=view&teaser=1&rtable=1&tab=0">Test ' .$wz->formName.'</a>'/*printDate($dvals['entry_date'])*/ , '', $m, "$m.$a");
		$titleBlock->addCrumb("?m=projects", $AppUI->_('list projects'));
		...
		$titleBlock->addCrumb('?m=wizard&a=form_useed&fid=' . $fuid . '&todo=addedit&idIns='.$idIns, $AppUI->_('edit record'));
		...
		$tabint = 'cellpadding="4"';	
		if($xmode === 'view')
			$tabint = 'cellpadding="2"';
		$blist .= '<!--cellpadding="4"  cellpadding="2"--><table width="100%" cellspacing="0"  border="0" class="std">
			<tbody><tr>
				<td width="100%" valign="top" class="mtab">
					<table cellspacing="1" '.$tabint.'>
						<tbody>';
	}
	if($xmode === 'add'){
	...
	if ($useID > 0 || $xmode === 'add') {
		...
		if($xmode === 'view'){
			if($wz->parent && $wz->multiplicity()!="ManyToMany"){
				$wz1 = new Wizard('print');
				$wz1->loadFormInfo($wz->parent);
				
				$blist .= '<tr><td colspan="4"><strong><a href="?m=wizard&a=form_use&fid='.$wz->parent.'&idIns='.$dvals['ref'].'&todo=view&teaser=1&rtable=1&tab=0">'.$wz1->formName.'</a></strong></td></tr>';
				$digestRel = $wz->getDigestRel();
				$countDigestRel = count($digestRel);
				if($countDigestRel>0){
					$queryFld = join(',', $digestRel);
					if($queryFld){
						$q = new DBQuery();
						$q->addTable('wform_'.$wz->parent);
						$q->addQuery($queryFld);
						$q->addWhere('id='.$dvals['ref']);
						$resultT = $q->loadList();
						foreach ($digestRel as $fld){
							$fldinfo = $wz1->findFieldName($fld);
							$blist .= '<tr><td>'.$fldinfo['name'].'</td><td colspan="3"><strong><a href="?m=wizard&a=form_use&fid='.$wz->parent.'&idIns='.$dvals['ref'].'&todo=view&teaser=1&rtable=1&tab=0">'.$resultT[0][$fld].'</a></strong></td></tr>';
						}	
					}
					
				}
				if($wz1->parent && $wz1->multiplicity()!="ManyToMany"){
					
					$q->addTable('wform_'.$wz->parent);
					$q->addQuery('ref');
					$q->addWhere('id='.$dvals['ref']);
					$resultRef = $q->loadResult();
					$wz2 = new Wizard('print');
					$wz2->loadFormInfo($wz1->parent);
				
					$blist .= '<tr><td colspan="4"><strong><a href="?m=wizard&a=form_use&fid='.$wz1->parent.'&idIns='.$resultRef.'&todo=view&teaser=1&rtable=1&tab=0">'.$wz2->formName.'</a></strong></td></tr>';
					$digestRel = $wz1->getDigestRel();
					$countDigestRel = count($digestRel);
					if($countDigestRel>0){
						$queryFld = join(',', $digestRel);
						if($queryFld){
							$q = new DBQuery();
							$q->addTable('wform_'.$wz1->parent);
							$q->addQuery($queryFld);
							$q->addWhere('id='.$resultRef);
							$resultT = $q->loadList();
							foreach ($digestRel as $fld){
								$fldinfo = $wz2->findFieldName($fld);
								$blist .= '<tr><td>'.$fldinfo['name'].'</td><td colspan="3"><strong><a href="?m=wizard&a=form_use&fid='.$wz1->parent.'&idIns='.$resultRef.'&todo=view&teaser=1&rtable=1&tab=0">'.$resultT[0][$fld].'</a></strong></td></tr>';
							}
						}
			...
			if (isset($fld['otm']) && count($fld['subs']) > 0) {
				...
				if ($fld['otm'] === true) {
					...
					foreach ($fld['subs'] as $sid => &$fsub) {
						...
						$pdfhtml .=str_replace("##CLASS##", '', $row);
						$blist .= str_replace("##CLASS##", $classv, $row);
						/* if($xmode==='view')
							$pdfhtml .= $wz->outputField(str_replace('fld_', '', $fsub['dbfld']), $fsub, $sendVal); */
						//$pdfhtml .= str_replace("##CLASS##",'',$wz->outputField($fld_id,str_replace('fld_', '', $fsub['dbfld']), $fld, $dvals[$fld['dbfld']]));
				...
				if (($xmode === 'view' && $fld['type'] !== 'entry_date') || $xmode !== 'view') {
					...
					if(($compterp % 2) === 1 && $fld['type'] != 'note' &&  (isset($temp['type']) && $temp['type'] != 'note')){
						...
						$row = str_replace("##CLASS##", $classv, $row);
						$blist .= $row;
						if($fld['type'] != 'note' || (isset($temp['type']) && $temp['type'] != 'note') && (isset($temp['otm']) && !$temp['otm']))
							$compterp++;
					}
					$pdfhtml .= str_replace("##CLASS##",'',$wz->outputField($fld_id,str_replace('fld_', '', $fld['dbfld']), $fld, $dvals[$fld['dbfld']]));
		...
		if ($xmode === 'view'){
			if($wz->isRegisterForm($fuid)){
				...
				$lothact = '. <strong><a href="">'.$AppUI->_('Add to other activity').'</a><strong>';
				if(!$result){
					$result = 'Nothing';
					$lothact = ' <a href="">'.$AppUI->_('Add to activity').'</a>';
				}
				//$blist .= '<td style="width:50%" class="hilite" align="left">'.$result.$lothact.'<td></tr>';
		...
		if(count($sections)>0){
			...
			foreach ($sections as $section_id => $section){
				...
				$headsind = 0;
				$fld = $section;
				$blist .= '<table><tr class="hidefp"><td><h3 style="margin:0px">'.$section['name'].'</h3></td>
							<td colspan="3">';
				$blist .= "<table border='0' cellpadding='2' cellspacing='2' class='tbl usub'>
					<thead><tr>";

				$allsection .= "<br/><table border='0' cellpadding='0' cellspacing='0'>
					<tr><td colspan='".count($fld['subs'])."' style='border: 1px solid black;'><h3 style='margin:0px'>".$section['name']."</h3></td></tr><tr>";
				
				
				foreach ($fld['subs'] as &$fsub) {
					$headsind++;
					$blist .= "<th>" . $fsub['name'] . "</th>";
					$allsection .= "<td style='border: 1px solid black;'><b>" . $fsub['name'] . "</b></td>";
					/* if($xmode==='view')
					 $pdfhtml .= "<th>" . $fsub['name'] . "</th>"; */
				}
				$allsection .= '</tr>';
				...
				if (count($subRowSet) > 0) {
					...
					foreach ($subRowSet as $sy => &$srset) {
						...
						foreach ($fld['subs'] as $sid => &$fsub) {
							$tlist .= $wz->outputField($fld_id,$fldprefix . '[' . $sy . '][' . $fsub['dbfld'] . ']', $fsub, $srset[$fsub['dbfld']], true);
							$allsection .= str_replace("<td",'<td style="border: 1px solid black;"',$wz->outputField($fld_id,$fldprefix . '[' . $sy . '][' . $fsub['dbfld'] . ']', $fsub, $srset[$fsub['dbfld']], true));
	...
	if ($xmode === 'view'){
		$pdfhtml .= '</table>'.$allsection.'</div></body></html>';
		...
		$mult = $wz->getMultiplicity($fuid);
		if($mult == "ManyToMany"){
			$q = new DBQuery();
			$q->addTable('wf_'.$fuid.'_wf_'.$wz->parent);
			$q->addQuery('COUNT(*)');
			$q->addWhere('id='.$idIns);
			$res = $q->loadResult();
			$tpos=$tabBox->add ( $dPconfig ['root_dir'] .'/modules/wizard/vw_wizard', $AppUI->_('List').' ('.$res.')' );
			$_SESSION['wiz_tab'][$tpos]=$wz->parent;
			$tablequery = 'wf_'.$fuid.'_wf_'.$wz->parent;
			$_SESSION['record_id'] = $idIns;
			$sender = $wz;
		}else{
			$q = new DBQuery();
			$q->addTable('form_master');
