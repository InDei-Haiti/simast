...
class Wizard {
	...
	function loadFormInfo($id) {
		$this->formID = $id;
		$sql = 'select title,fields,parent_id,digest,digestrel,valid,subs,registry,task_id,multiplicity,forregistration from form_master where id="' . $this->formID . '" limit 1';
		$res = mysql_query($sql);
		if ($res) {
	...
	function drawDigest($idfw,$ref,$page,$epp=false,$view_table=true) {
		global $m,$a,$tab,$AppUI,$tablequery,$querylquery,$querylfilter;
	    echo '<pre>';
		var_dump($_GET);
		echo '</pre>';
		//exit;
			...
			foreach ($this->digestrel as $index => $field){
				if($field){
					...
					if(isset($nitem["raw"]["sysv"])){
						$fieldsFilter['rel_'.$field] = array("title"=>$nitem["name"],"type"=>$nitem["raw"]["type"],"sysv"=>$nitem["raw"]["sysv"]);
					}else{
						$fieldsFilter['rel_'.$field] = array("title"=>$nitem["name"],"type"=>$nitem["raw"]["type"],"sysv"=>$nitem["raw"]["sysv"]);
					}
				}
		...
		if($count > 20){
			...
			$rh .= '<form id="filterform" name="filterform" action="'.$link.'" method="post">
					<fieldset id="filters" style="margin-left:-7px; width: 1300px" class="collapsible '.$filcollapse.' header_collapsible">
			...
			$rh .= '<fieldset id="options" style="margin-left: -7px; width: 1300px"
								...
								<td style="width:1%;margin: 0;padding: 0;border: 0;outline: 0;font-size: 100%;vertical-align: baseline;background: transparent;">
									...
									<select id="selected_columns" multiple="multiple" name="selected_columns[]" size="10" style="min-width:150px">';
										...
										if(count($selected_columns)>0){
											...
											foreach ($this->digestrel as $index => $field){
												if(trim($field) != "")
													$rh .= '<option value="'.'rel_'.$field.'">'.$fieldsFilter['rel_'.$field]["title"].'</option>';
											}
		...
		if($searchlike){
			...
			$sql = 'SELECT * FROM  ' . $this->tableName.$jointure.' WHERE '.$searchlike.$wiz_ref.$user_fil.$querylfilter.' ORDER BY ' . $this->tableName.'.entry_date DESC'.'  limit '.$start.','.$epp;
			/* if($ref){
			...
			$sql = 'SELECT * FROM  ' . $this->tableName.$jointure.' WHERE '.$filter.$wiz_ref.$user_fil.$querylfilter.' ORDER BY ' . $this->tableName.'.entry_date DESC'.'  limit '.$start.','.$epp;
		}else{
			$wiz_ref = '';
			$user_fil = '';
			if(isset($_GET['wiz_ref'])){
				...
				$wiz_ref = ' WHERE ref='.$_GET['wiz_ref'];
				if(isset($_GET['user']) && $_GET['user']==='me'){
					$wiz_ref .= ' AND '.$this->tableName.$jointure.'.user_creator='.$AppUI->user_id;
				}
				...
				if(isset($_GET['user']) && $_GET['user']==='me'){
					$user_fil = ' WHERE '.$this->tableName.$jointure.'.user_creator='.$AppUI->user_id;
					if($tablequery){
						$querylfilter = ' AND id IN('.$querylquery.') ';
					}
				}
			...
			$sql = 'SELECT * FROM  ' . $this->tableName.$jointure.$wiz_ref.$user_fil.$querylfilter.' ORDER BY ' . $this->tableName.'.entry_date DESC'.' limit '.$start.','.$epp;
		...
		if($this->forregistration){
			$rhb .= '<input type="button" value="'.$AppUI->_('Import').'" onClick="dialogNewClient('.$idfw.');">&emsp;&emsp;';
			$rhb .= '<input type="button" value="'.$AppUI->_('Add New').'" onClick="window.location=\'./index.php?m=wizard&a=form_useed&fid=' . $this->tableID . '&todo=addedit\'">';
		}else if($m !='tasks' && $ref){
			if($this->multiplicity==='One'){
				...
				if($result==0)
					$rhb .= '<input type="button" class="button" value="'.$AppUI->_('Add New').'" onClick="window.location=\'./index.php?m=wizard&a=form_useed&fid=' . $this->tableID . '&todo=addedit'.$reflink.'\'">';
			...
			if($this->multiplicity==='One'){
					$rhb .= '<input type="button" class="button" value="'.$AppUI->_('Add New').'" onClick="window.location=\'./index.php?m=wizard&a=form_useed&fid=' . $this->tableID . '&todo=addedit'.''.'\'">';
				
			}elseif($this->multiplicity==='Many'){
				$rhb .= '<input type="button" class="button" value="'.$AppUI->_('Add New Entry').'" onClick="window.location=\'./index.php?m=wizard&a=form_useed&fid=' . $this->tableID . '&todo=addedit\'">';
			}
		}elseif($m == 'wizard'){
			if($ref){
				$reflink = '&parent_id='.$ref;
