...
function resultBuilder($qmode) {
	...
	$dPconfig,$jsonmap,$where_query,$join_query,$querysave,$countall;
	$lvdForm = false;
	$project_id = null;
	...
	if(isset($_POST['form'])){
		...
		foreach ($_POST['form'] as $table => $arr_fields){
			...
			/* $sources['key_ui'] =  $wz->title;
			$headers['key_ui']['title'] = 'key';
			$headers['key_ui']['type'] = 'string';
			$sels[] = 'string';
			$headers['key_ui']['table'] = $table;
			$headers['key_ui']['form'] = $formId; */
			...
			$arr_fields = $table.'.id as '.$table.'_id,'.$arr_fields;
	...
	if($res){
		...
		if ($qmode != 'mas') {
			...
			<input type="button" class="button adcbutt" value="Build Maps" onclick="gpgr.mapss();">';
		}
		...
		<input type="button" class="button adcbutt" value="Save To File" onclick="gpgr.saveToFile()"  class="button adcbutt">
		</form></div>';
		$tfhtml = '<ul>';
		...
		$tfhtml .= '</ul>';
		 ...
		 if($page==1){
		 	...
		 	//$pageURL .= "&p=".($page - 1);
		 	$prev = '<a href="'.$pageURL."&token=".$token."&p=".($page - 1).'&epp='.$epp.'"><h3>Prev</h3></a>';
		 	if($page<$nbPages)
		 		$next = '<a href="'.$pageURL."&token=".$token."&p=".($page + 1).'&epp='.$epp.'"><h3>Next</h3></a>';
		 	else
		 		$next = '<h3 style="color:#c0c0c0">Next</h3>';
		...
		$rhtml .= '</div>';
		$rhtml .= $rf;
		$tab_start = '<div id="mholder"><table class="rtable moretable" id="rtable" border="0" style="display:block;" cellpadding="2" cellspacing="1">';
		$tab_head = "\n<thead><tr>";
		$colz = '<colgroup>';
		$ind = 0;
		$addcl = '';
		...
		while ( $rowdata = db_fetch_assoc ( $res ) ) {
			...
			foreach ( $headers as $hcode => $hcodeArray ) {
				...
				if($firstTD){
					...
					$row .= "<td class='vcell text-left' func-id='".$funcid."' func-action='".$func."' data-value='".$tempforStore."' parent-id='".$parentid."' mode='".$mode."'>" . strval($forStore) . "</td>\n";
					if($m==='manager'){	
