...
var pager = function(){
	...
	this.patchCell = function(/*cell,y, x,*/cell, dval,val,vval/*,cw*/){
		...
		console.log('mode=patch&x='+x+"&y="+y+"&val="+dval+"&fid="+fid+"&fld="+fld+"&did="+did+"&vval="+vval+'&vmd='+vprmode);
		$j.ajax({
			type: 'post',
    ...
    this.collector = function(mode){
		...
		for(var hid=0; hid < thl;hid++){
			...
			.bind("click", {
            	...
            	//console.log($j(this).closest("th").index());
	...
	this.saveToFile = function(){
		...
		$dbox = $j("<div id=\'dbaddf\' title=\'Save To File\'></div>");
		 $dbox.append("<table>" +
				 "<tr><td>File Name</td><td><input type='text' id='fnamefile'/></td></tr>"+
...
//checks = */console.log($(\'#rtable\').find(\'input[type=\"checkbox\"]:checked\'));/*if(checks!=null){$('#btn_add_s').show();}else{$(\'#btn_add_s\').hide();}*/\
