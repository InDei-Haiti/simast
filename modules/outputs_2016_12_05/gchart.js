		...
		$j.get("/?m=outputs&a=gchart&suppressHeaders=1", {mode: "loadforms", pid: project_id}, function (msg) {			
			if (msg && msg !== 'fail') {				
				...
				$.each(msg, function(i){
					...
					var li = '<li>'+'<div class="switch" id="switch'+msg[i].id+'" style="background-position: 0px -144px;"></div>'+msg[i].title;
					...
					if(fields){
						...
						$.each(fields.fields, function(key,val){
							...
							var vli = '<tr style="padding:5px"><td align="left" style="width:30%;margin:15px;background:transparent" valign="middle" class="hilite"><label><input type="checkbox" class="jcheck" name="form[wform_'+msg[i].id+'][]" value="'+key+'"/>'+key+'- '+val.title+'</label>'+tempval+'</td><td>';
						...
						if(fields.section){
							$.each(fields.section, function(key,val){
								var classname = key;
								var li = '<tr><td valign="top" colspan="2">'+val['name']+'</td></tr>';
								var ftext = 'fields';
								var ntext = 'name';
					...
					if(msg[i].parent_id!='0'){
						...
						var li1 = '<li><div class="switch" id="switch'+msg[i].id+'_'+msg[i].parent_id.id+'" style="background-position: 0px -144px;"></div>'+msg[i].parent_id.title;
					...
					$j("#switch"+msg[i].id).live("click", function () {
						if($j("#panel"+msg[i].id).is(':visible')){
							...
							$j("#switch"+msg[i].id).attr('style','background-position: 0px -144px;');
						}else{
							$j("#panel"+msg[i].id).show();
							$j("#switch"+msg[i].id).attr('style','background-position: -12px -144px;');
						}
					});
				});
			} else {				
