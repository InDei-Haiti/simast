...
function buildLocations(){
	...
	$q->addQuery('ar.region_id, ar.region_name, ar.region_parent, ar.region_level,gps');
	...
	if (file_exists('./templates/locations.tpl')) {
		...
		$_1sts = array_filter($admin_regions, "toop");
		foreach($_1sts as $_1st) {
			$parent1 = $_1st['region_id'];
			$_2nds = array_filter($admin_regions, "toop2");
			$locations .= /*(count($_2nds))
			? '<li><input type="checkbox"><label>' . $_1st['region_name'] . '</label>*/
			//:
			'<li><input type="checkbox" class="last" id="'.$_1st['region_id'].'"><label>' . $_1st['region_name'] . '</label>'.
			(count($_2nds)  > 0 ? '<ul>' : '</li>');
			$dump[$_1st['region_id']]=array('level'=>$_1st['region_level'],'gps'=>$_1st['gps'],'parent'=>$_1st['region_parent'],'title'=>$_1st['region_name']);
			foreach($_2nds as $_2nd) {
				$parent2 = $_2nd['region_id'];
				$_3rds = array_filter($admin_regions, "toop3");
				$locations .= /*(count($_3rds))
				? '<li><input type="checkbox"><label>' . $_2nd['region_name'] . '</label><ul>'
				: */
				'<li><input type="checkbox" class="last" id="'.$_2nd['region_id'].'"><label>' . $_2nd['region_name'] . '</label>'.
				(count($_3rds)  > 0 ? '<ul>' : '</li>');
				$dump[$_2nd['region_id']]=array('level'=>$_2nd['region_level'],'gps'=>$_2nd['gps'],'parent'=>$_2nd['region_parent'],'title'=>$_2nd['region_name']);
				foreach($_3rds as $_3rd) {
					$parent3 = $_3rd['region_id'];
					$_4ths = array_filter($admin_regions, "toop4");
					$locations .= /*(count($_4ths))
					? '<li><input type="checkbox"><label>' . $_3rd['region_name'] . '</label><ul>'
					: */
					'<li><input type="checkbox" class="last" id="'.$_3rd['region_id'].'"><label>' . $_3rd['region_name'] . '</label>'.
					(count($_4ths)  > 0 ? '<ul>' : '</li>');
					$dump[$_3rd['region_id']]=array('level'=>$_3rd['region_level'],'gps'=>$_3rd['gps'],'parent'=>$_3rd['region_parent'],'title'=>$_3rd['region_name']);
					foreach($_4ths as $_4th) {
						$locations .= '<li><input type="checkbox" class="last" id="' . $_4th['region_id'] . '"><label>' . $_4th['region_name'] . '</label></li>';
						$dump[$_4th['region_id']]=array('level'=>$_4th['region_level'],'gps'=>$_4th['gps'],'parent'=>$_4th['region_parent'],'title'=>$_4th['region_name']);
					}
					if (count($_4ths))
						$locations .= '</ul></li>';
				}
		...
		file_put_contents('./templates/location_dump.php','<?php $locData = '.var_export($dump,true).';');
	}
	return $locations;
}
