...
jQuery.tblFilter = function(tbl,tblOptions,colOptions){
    if (jQuery("#tableFilter_header",tbl).size()){return};
	var root = this;
	
    this.version = "1.0 b2";
	this.userSettings = (jQuery.cookie) ? (JSON.parse(jQuery.cookie('daemach.tf.userSettings')) || {}):{};
	...
	jQuery(window).bind("resize",function(e){ 
	    ...
	    jQuery('#'+root.menu.className+'_trigger')[0].offsetLeft+"px" 
	});
	
	// and on the initial load
	jQuery(window).bind("onLoad",function(e){ jQuery('#'+root.menu.className).css("left",function(){return jQuery('#'+root.menu.className+'_trigger')[0].offsetLeft+"px"}); });
    ...
    prototype: {
        filterTable: function(op){
            ...
            for (var r=0; r<ar.length; r++) {
                ...
                if (show && !this.browserSucks) {
                    row.style.display = "";
                    if (!(fc++%2)) {
                        jQuery(row).addClass(sc);
                        } else {
                        jQuery(row).removeClass(sc);
                    };
                } else {
            ...
            if (this.browserSucks){
                for (var i=0;i<ar.length;i++){
                    ...
                    if (ar[i][2]){
                        e.style.display = "";
                        if (!(fc++%2))
                        jQuery(e).addClass(sc);
                        else
                        jQuery(e).removeClass(sc);
                        } else {
        ...
        pageNav: function(e){
            ...
            switch (action){
                ...
                break;
                case "Current":
                val = parseInt(jQuery('#pageCurrent').val());
                ...
                break;
                case "Records":
                val = parseInt(jQuery('#pageRecords').val());
        ...
        updatePageNav: function (v,t,l){
            ...
            jQuery("#pageFirst").add("#pagePrev").attr("src",function(){ return (root.maxPage === 1 || root.currentPage == 1) ? disable(this.src) : enable(this.src); });
            jQuery('#pageCurrent').val(this.currentPage.toString());
            jQuery("#pageLast").add("#pageNext").attr("src",function(){ return (root.maxPage === 1 || root.currentPage == root.maxPage) ? disable(this.src) : enable(this.src); });
            jQuery('#pageCount').text(this.maxPage.toString());
            jQuery('#pageVisible').text(v.toString());
            jQuery('#pageTotal').text(t.toString());
            jQuery('#p_menu').css("left",function(){return jQuery('#p_menu_trigger')[0].offsetLeft+"px"});
        },
		menu: {
			count: 0,
			isAnimating: false,
        ...
        createMenu: function(id, base, container, direction, trigger, inEvent, outEvent,menuClass){
			...
			base = jQuery(base).css("position","relative");
            ...
            menu = jQuery('<div id="'+id+'" class="menu '+menuClass+'"><ul id="'+id+'_root"></ul></div>').appendTo(base);
			...
			if (trigger){
                ...
                jQuery(trigger).bind(inEvent,function(){
					if (!root.menu.isAnimating){
						...
						jQuery('#'+id).slideToggle("fast",function(){root.menu.isAnimating = false;});
                ...
                jQuery('#'+id).truemouseout(function(){
					if (!root.menu.isAnimating){
						jQuery(this).slideUp("fast",function(){root.menu.isAnimating = false;});
						root.menu.isAnimating = true;
					}
					return false;
				});
        ...
        createSubMenu: function(text,target){
            ...
            .appendTo(target).hover(function(){jQuery(this).addClass("over")},function(){jQuery(this).removeClass("over")})
    ...
    prototype : {
        ...
        addField: function(target,type,options,numeric){
            ...
            switch (type){
                ...
                .bind("click",this,function(e){ e.data.root.filterFocus(e,this); })
                .val("");
                jQuery(target).attr("nowrap","nowrap");
                break;
            };
