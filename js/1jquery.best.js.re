...
(function (a) {
    ...
    a.fn.sSelect = function (b) {
        return this.each(function () {
            var j = {defaultText: "Please select", animationSpeed: 0, ddMaxHeight: ""};
            ...
            if (e.children("optgroup").length == 0) {
                ...
                e.children("optgroup").each(function (D) {
                    var B = a(this).attr("label"), E = a('<li class="newListOptionTitle">' + B + "</li>");
                    E.appendTo(A);
                    var C = a("<ul></ul>");
                    C.appendTo(E);
            ...
            function q() {
                var C = s.offset().top, B = jQuery(window).height(), D = jQuery(window).scrollTop();
                if (p > parseInt(m.ddMaxHeight)) {
                    p = parseInt(m.ddMaxHeight)
                }
                C = C - D;
                if (C + p >= B) {
                    A.css({top: "-" + p + "px", height: p});
                    e.onTop = true
                } else {
                    A.css({top: o + "px", height: p});
                    e.onTop = false
                }
            }
            ...
            function i(D, F) {
                var B = s.offset().top, G = y.eq(D).offset().top, C = A.scrollTop();
                if (e.onTop == true) {
                    e.liOffsetTop = (((G - B) - o) + C) + parseInt(m.ddMaxHeight)
                } else {
...
jQuery.fn.center = function (absolute) {
    return this.each(function () {
        ...
        t.css({
            ...
            top: '50%',
            zIndex: '99'
        }).css({marginLeft: '-' + (t.outerWidth() / 2) + 'px', marginTop: '-' + (t.outerHeight() / 2) + 'px'});
        if (absolute) {
            t.css({
...
$j.fn.fixPosition = function () {
    $j(this).fadeTo(0).show();
    var meoff = $j(this).offset(), mw = $j(this).width(), mh = $j(this).height(), wwid = $j(window).width(), wswid = $j(window).scrollWidth, whgt = $j(window).height(), nx, ny;
    if ((meoff.left + mw) > wwid) {
        nx = meoff.left - mw - 20;
    } else {
        nx = meoff.left;
    }
    if ((meoff.top + mh) > whgt) {
        ny = meoff.top - mh - 20;
    } else {
        ny = meoff.top;
    }
    $j(this).css({top: ny, left: nx});
    $j(this).fadeTo('slow', 100);
    return this;
}
 ...
 * Time: 19:02:33
 */
function form2object(rootNode, delimiter) {
    ...
    $.fn.extend({
        ...
        getParams: function (convert) {
            ...
            this.each(function () {
                ...
                var key = el.name, value = $.fn.val.call([el]) || $.data(el, "value"), isRadioCheck = radioCheck.test(el.type), parts = key.match(keyBreaker), write = !isRadioCheck || !!el.checked,//make an array of values
                    lastPart;
                if (convert) {
                    if (isNumber(value)) {
    ...
    a.fn.sSelect = function (m) {
        return this.each(function () {
            function n() {
                ...
                h -= b;
                h + k >= c ? (f.css({height: k}), i.css({
                    top: "-" + k + "px",
                    height: k
                }), e.onTop = !0) : (f.css({height: k}), i.css({top: s + "px", height: k}), e.onTop = !1)
            }
            ...
            var o = a.extend({
                ...
                ddMaxHeight: "",
                containerClass: ""
            }, m), e = a(this), l = a('<div class="selectedTxt"></div>'), d = a('<div class="newListSelected ' + o.containerClass + '"></div>'), i = a('<div class="SSContainerDivWrapper" style="visibility:hidden;"></div>'), f = a('<ul class="newList"></ul>'), w = -1, c = -1, p = -1, q = [], v = !1, r = !1, g;
            ...
            l.bind("click.sSelect", function (b) {
                ...
                a(".SSContainerDivWrapper").not(a(this).next()).hide().parent().css("position", "static").removeClass("newListSelFocus");
                i.toggle();
...
function moreless() {
    $(".detail").live("click", function () {
        var $block = $(this).closest(".moreless"), btxt, atxt, nstate;
        if ($block.attr("data-state") == 'off') {
            nstate = 'on';
            btxt = 'less';
            atxt = $block.attr("data-tbody");
        } else {
            nstate = 'off';
            btxt = 'more';
            atxt = $block.attr("data-teaser");
        }
        $block.attr("data-state", nstate)
            .find(".teasert").text(atxt).end()
            .find(".detail").text(btxt);
    });
    $(".moreless").each(function (i) {
        var intxt = $(this).text(), ttt = intxt.substr(0, 100) + '...';
        if (intxt.length > 100) {
            $(this)
                .attr("id", "more_" + i)
                .attr({
                ...
                .append("<span class='teasert'/>")
                .find(".teasert").text(ttt).end()
                .append("<span class='detail'>more</span>");
        }
    });
}

function verboseWindow(el) {
    $("<div>", {id: "iwindow"}).appendTo(document.body);
    $(el + "").live({
        mouseenter: function () {
            var os = $(this).attr("data-detail"), pof = $(this).offset(),
                wrapit = $(this).hasClass("inwrap");
            if (os && os.length > 0) {
                $("#iwindow")
