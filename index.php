...
require_once (DP_BASE_DIR . '/functions/admin_func.php');
...
//echo dPgetParam($_GET, 'suppressHeaders', false);
...
//echo getCoordinates("Ouest,Haiti");
 function initLocationDepartment(){
	$q = new DBQuery();
	...
	while( $row = db_fetch_assoc ( $res ) ) {
		if($row['administration_com']!=='999999'){
			$resloc22 = getCoordinates($row['administration_com_name'].',Haiti');
			//echo $resloc22."<br/><br/>";
			if($resloc22){
				echo $resloc22."<br/><br/>";
				$resloc22 = explode(",", $resloc22);//split(",", $resloc);
				var_dump($resloc22);
				$lat = $resloc22[0];
				$lng = $resloc22[1];
				//echo $lat .' '.$lng."<br/><br/>";
				if($lat && $lng){
					$q->clear();
					$q->addTable("administration_com");
					$q->addUpdate("administration_com_lat", $lat);
					$q->addUpdate("administration_com_lng", $lng);
					$q->addWhere('administration_com_code="'.$row['administration_com_code'].'"');
					$sql = $q->prepare();
					echo ' '.$sql."<br/><br/>";
					db_exec($sql);
				}
...
if (isset($_REQUEST['login'])) {
	if(isset($_REQUEST['clir'])){
		$username = dPgetCleanParam($_GET, 'username', '');
		$password = dPgetCleanParam($_GET, 'password', '');
...
// Turning echo of header php OFF if module = projects or it`s ajax query (moduile == 'ajax')
if (($m == 'projects') && !$oldtemplate) {
...
$tstatus = dPgetSysVal( 'TaskStatus' );
//echo DP_BASE_DIR.'/functions/' . $m . '_func.php';
...
if ($u && file_exists(DP_BASE_DIR . '/modules/' . $m . '/' . $u . '/' . $u . '.class.php')) {
	...
	//echo DP_BASE_DIR . '/modules/' . $m . '/' . $u . '/' . $u . '.class.php';
}
...
if (file_exists($module_file)) {
	...
	echo $AppUI->_('The module is coming soon "' . $m . '" wait!');
}
// wtf??  why?
if (!$suppressHeaders) {
