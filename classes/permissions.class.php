...
/*======================================================================*\
  ...
  function getAcoIdByValue($aco_value){
  	$q = new DBQuery();
  	$q->addTable('gacl_aco');
  	$q->addQuery('id');
  	$q->addWhere('value="'.$aco_value.'"');
  	$res = $q->loadResult();
  	return $res;
  }
  function checkForm($user_id,$module,$form,$option){
  ...
  function getFormPermission($user_id,$module){
  ...
  function checkRoleForm($role_id,$module,$form,$option){
  
  	$option = $this->getAcoIdByValue($option);
  	$q = new DBQuery();
  	$q->addTable('permission_role_form');
  	$q->addQuery('status');
  	$q->addWhere('role_id='.$role_id);
  	$q->addWhere('form='.$form);
  	$q->addWhere('module="'.$module.'"');
  	$q->addWhere('find_in_set("'.$option.'",replace(type, " ", ","))');
  	$res = $q->loadResult();
  ...
  function getFormRolePermission($role_id,$module){
