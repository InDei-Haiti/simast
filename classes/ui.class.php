 ...
 */
	...
	/**
	 * Load file - member of class - sub class
	 * @param string $name The name of non-module class file
	 */

	function getFileInModule($name,$fname){
		global $baseDir;
		if ($name != '' && $fname != '') {
			return "$baseDir/modules/".$name."/".$fname.".php";
		}
	}
...
*/
	function setUserLocale($loc='', $set = true) {
		...
		if (! $loc) {
			...
			$loc = ((@$this->user_prefs['LOCALE']) ? $this->user_prefs['LOCALE'].'pui'
 ...
 */
	...
	function __($str, $flags = 0) {
		...
		return stripcslashes($str);//$this->___($str, $flags);
	}
	...
	function ___($str, $flags = 0) {
		...
		switch ($flags & UI_OUTPUT_MASK) {
			case UI_OUTPUT_URI:
				...
				break;
			case UI_OUTPUT_HTML:
				$str = htmlentities(stripslashes(utf8_encode($str)), ENT_COMPAT, $locale_char_set);
				//$str = str_replace('&#039;', '&apos;', $str);
				break;
			case UI_OUTPUT_JS:
...
*/
	function login($username, $password) {
		...
		$q  = new DBQuery;
		$q->addTable('users');
		$q->addQuery('user_id,user_contact, contact_first_name as user_first_name, contact_last_name as user_last_name, contact_company as user_company, contact_department as user_department, contact_email as user_email, user_type');
...
// load the user preferences
		...
		$this->setUserLocale();
		$this->checkStyle();
		if((int)$_POST['qnr_mode'] === 0){
			$_SESSION['qnr_mode'] = false;
		}else{
			$_SESSION['qnr_mode'] = true;
		}
		return true;
	}
/************************************************************************************************************************
 ...
 */
	...
	*/
	...
	function userRoles(){
		$perms =$this->acl();
		$user_roles = $perms->getUserRoles($this->user_id);
		return $user_roles;
	}
	...
	function show($extra='', $js_tabs = false) {
		...
		if ($this->active < 0 || @$AppUI->getPref('TABVIEW') == 2) {
			...
			foreach ($this->tabs as $k => $v) {
				...
				$s .= "\n\t" . '</td>';
				$s .= "\n\t" . '<td id="toptab_' . $k . '" width="1%" nowrap="nowrap" nnnnnnnnnnnnn';
				if ($js_tabs) {
					$s .= ' class="' . $class . '"';
				}
				$s .= '>';
...
class CTitleBlock_core {
	...
	function show() {
		...
		if ($this->icon) {
			$s .= "\n" . '<td width="42">';
			$s .= dPshowImage(dPFindImage($this->icon, $this->module),60,60);
			$s .= '</td>';
		}
		$s .= ("\n" . '<td align="left" width="100%" nowrap="nowrap"><h3 style="padding-bottom: 10px">'
		       . $AppUI->_($this->title) . '</h3></td>');
...
class FormExtractor{
	private   $table = 'form_master';
	protected $areaTypes;
	protected $areaName;
	protected $areaKey;
	protected $allForms = array();
	protected $localCache = array();
	protected $formKeyTitle = array();

	function __construct(){
		$this->collectTypesInfo();
	}

	private function collectTypesInfo (){
		$this->areaTypes = dPgetSysValSet('FormConnection');
		$this->areaKey = array_search($this->areaName,$this->areaTypes);
	}
	...
	public function getAvailableItems (){
		if(count ($this->allForms) === 0){
			...
			$q->addWhere('farea="'.$this->areaKey.'"');
			$this->allForms =$q->loadList();
		}
		foreach($this->allForms as &$item){
			$this->formKeyTitle[$item['id']] = $item['title'];}

			return $this->allForms;
	}

	/**
	 * Build html OPTION set with list of avaliable dynaforms
	 * @param string $defaultValue preselect value in dropdown list
	 * @return string
	 */
	public function printOptionList ($defaultValue = ''){
		...
		foreach($this->allForms as $item){
			...
			$code.= "<option {$selTxt} value='{$item['id']}'>{$item['title']}</option>";
		}
		return $code;
	}
	...
	public function getFormName ($formId){
		...
		if ((int)$formId > 0){
			$this->getAvailableItems();
			$result = $this->formKeyTitle [$formId];
		}
		return $result;
	}

	public function getUsedForm($varId){}

	public function getInstances ($dsetId){
		return '';
	}
}
class DataSets extends FormExtractor{
	function __construct(){
		$this->areaName = 'Data sets';
		parent::__construct();
		$this->localCache[$this->areaName] = array();
	}

	public function getUsedForm($datasetId, $want = 'title', $level = 1 ){
		if (!array_key_exists($datasetId,$this->localCache[$this->areaName])){
			$q = new dbQuery();
			$q->addTable('xdata_level_'.$level);
			$q->addWhere('id="'.(int)$datasetId.'"');
			$q->addQuery('forms');
			$q->setLimit(1);
			$formId = $q->loadResult();
			$this->localCache[$this->areaName][$datasetId] = $formId;
		}else{
			$formId = $this->localCache[$this->areaName][$datasetId];
		}
		...
		if($want == 'title'){
			$result = '';
			if((int)$formId > 0){
				foreach($this->allForms as &$fitem){
					if($fitem['id'] == $formId){
						$result = $fitem['title'];
					}
				}
	...
	/**
	 * Collect all the instances for specified dataSet
	 * @param $dsetId
	 * @return string|void
	 */
	public function getInstances($dsetId){
		$final = array();
		$sql = "select `instance_id` as id,`instance_name` as name from `xdata_level_2` where instance_dataset={$dsetId}";
		$res = my_query($sql);
		if ($res && my_num_rows($res) > 0){
			while($row = my_fetch_assoc($res)){
				$final[$row['id']] = $row['name'];
			}
		}
		return $final;
	}
