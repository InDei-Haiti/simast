...
function drawDateCalendar($name,$value,$hidden=false,$tags='',$yearCase = false,$length=20,$hvalue=false,$extraEvent='',$minDate='',$maxDate=''){
	...
	$html='<span style="white-space:nowrap;">
				<input type="text" class="text spCals classfld '.$mclass.'" name="'.$name. $visual . '" '.$tags.' value="'.$value.'" size="'.$length.'" readonly="readonly" '.$myID.'>
		  </span>';		
	$dateFormat = 'calObj';
	...
	$totalCals++;
	//echo 'var t=cloneThis('.$dateFormat.'); t.defaultDate='.$value.';'.join(" ",$addlim).'t.onClose=function(dates){'.$extraEvent.' };$j("#'.$hang.'").datepick( t );';
	return $html;
}
